...
\section{Overview}
A set of input parameters is called upon. From here a one dimensional grid $z$ is created that models the thickness of the biofilm on the tank walls with a specified number of grid points. The initial conditions for the substrate and biomass concentrations within the tank as well as biofilm thickness are specified from these input parameters. An initial time-step is specified in order to begin looping over the following process.


Within the time loop, the biofilm grid is updated to align $\Delta z$ to the current biofilm thickness $L_f^{t}$. The Biofilm Diffusion function is then called upon, from which a boundary flux term and substrate concentration array $S_b$ is produced. The Biofilm Thickness function then computes the new biofilm thickness $L_f^{t+1}$ and current detachment velocity $v_g^{t}$. The Tank Environment function then computes the new substrate and biomass concentrations within the tank as well as the new time-step to be used in the next iteration of the loop.
...
\begin{equation}
  \label{eq:vg_int}
  {v_g^{t}}={\int_{0}^{L_f^{t}}\mu(S^{t}_b(z))dz}.
\end{equation}
