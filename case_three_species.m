...
param.Title  = {'Three Species Case'};
param.SNames = {'Oxygen', 'Sulfate'}%, 'Sulfide'};
param.XNames = {'Phototroph', 'SRB'}%, 'SRB'};
...
% Biofilm
param.Nz=60;          % Linear grid points to describe biofilm
param.phibo= [0.2/2; 0.2/2]%  0.2/3]    % Biofilm particulates initial condition(s)
...
%param.light=@(t,z,Lf) max(0,param.I-(Lf - z)*param.diss);
...
param.mu=@(S,X,Lf,t,z,param) [
    ones(size(z)).*(mumaxA.*param.light(t,z,Lf));
    ones(size(z)).*(mumaxC*S(2,:))./(KmC2+S(2,:)).*(1./(1+S(1,:)/KI));
    %ones(size(z)).*(mumaxB*S(1,:))./(KmB1+S(1,:)).*(S(3,:))./(KmB3+S(3,:));
    ];
    %ones(size(z)).*(mumaxC*S(2,:))./(KmC2+S(2,:)).*(1./(1+S(1,:)/KI));
%];
