function [Lf,Vdet,Vg]= BiofilmThickness_Fn(Sb,Lf_old,mu,Kdet,mumax,dt,dz)
...
%Biofilm Thickness
Lf=Lf_old+dt*(mu(Sb)*Lf_old-Kdet*Lf_old^2); %New Biofilm thickness at instant
...
for i=1:length(Sb)-1
    Vg=Vg+dz*((mu(Sb(i))+mu(Sb(i+1)))/2); %trapezoidal integration method for
