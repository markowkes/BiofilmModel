...
function Sin = fSin(t,k,param)
    ...
    theavi = mod(t, param.Sin.period(k));
    Sin = param.Sin.f{k}(theavi);
...
%     if ~exist('param.instantaneousDiffusion','var')
...
%     end
    ...
    if ~isequal(size(param.mu(Stest,Xtest,Lftest,ttest,ztest,param)),[Nx Nz])
        error(['mu(Sb,Xb,t,z,param) returns a matrix of size '...
            ...
            'it should return a matrix of size ',num2str(Nx),' x ',num2str(Nz)])
    end
